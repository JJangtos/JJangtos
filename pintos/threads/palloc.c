...
/* Obtains a single free page and returns its kernel virtual
   ...
   available, returns a null pointer, unless PAL_ASSERT is set in
   FLAGS, in which case the kernel panics.
   하나의 비어 있는 페이지를 얻고, 그것의 커널 가상 주소를 반환한다.
   PAL_USER가 설정되어 있다면, 그 페이지는 사용자 풀에서 얻어진다.
   그렇지 않으면 커널 풀에서 얻어진다.
   FLAGS 안에 PAL_ZERO가 설정되어 있다면, 그 페이지는 0으로 채워진다.
   사용 가능한 페이지가 없다면, 널 포인터를 반환한다.
   단, FLAGS에 PAL_ASSERT가 설정되어 있다면 그 경우에는 커널이 패닉 상태에 빠진다 */
void *
