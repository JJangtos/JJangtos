 ...
 * uninit_initialize 함수는 해당 페이지를 특정 페이지 객체(anon, file, page_cache)로 변환(transmute)하며,
 * 해당 페이지 객체를 초기화한 후, vm_alloc_page_with_initializer 함수에서 전달된
 * 초기화 콜백(initialization callback)을 호출합니다.
 * */
 ...
 * 프로세스 종료 시 단 한 번도 참조되지 않은 uninit 페이지가 남아 있을 수 있습니다.
 * PAGE 자체는 호출자가 해제합니다. */
static void
